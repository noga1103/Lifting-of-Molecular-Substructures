...
from train.train_utils import DEVICE, ONE_HOT_0_ENCODING_SIZE, ONE_HOT_1_ENCODING_SIZE, WEIGHT_DTYPE, generate_x_0, generate_x_1
import torch
...
class CANModel(torch.nn.Module):
    ...
    def forward(self, graph):
        ...
        try:
            ...
            def pad_sparse(sparse_tensor, new_size):
                ...
                return torch.sparse_coo_tensor(indices=indices, values=values, size=(new_size, new_size), device=DEVICE).coalesce()
            ...
            if x_1.shape[0] < max_dim:
                padding = torch.zeros((max_dim - x_1.shape[0], x_1.shape[1]), dtype=x_1.dtype, device=x_1.device)
                x_1 = torch.cat([x_1, padding], dim=0)

            # Process through CAN layers
            x_1_current = x_1
            for layer in self.layers:
                x_1_current = layer(x=x_1_current, down_laplacian_1=down_laplacian_1, up_laplacian_1=up_laplacian_1)
    ...
    def add_graph_matrices(enhanced_graph):
        ...
        incidence_2_t = enhanced_graph.data.cell_complex.incidence_matrix(rank=2).T
        adjacency_0 = enhanced_graph.data.cell_complex.adjacency_matrix(rank=0)
